...
do
  ...
  repo=$(yq '.forgejo_role_installer_repository' "$doc")
  forgejo_url=$(yq '.forgejo_role_installer_url' "$doc")
  if [ -z "$forgejo_url" ] || [ "$forgejo_url" = "null" ]; then
  ...
  (
    echo "## ${bin_name}"
    echo ""
    echo "[Forgejo repository](${forgejo_url}/$repo)"
    echo ""
    echo '```'
...
do
  ...
  (
    echo "[![CI](https://github.com/sgaunet/ansible-role-forgejo-release-installer/workflows/CI-${bin_name}/badge.svg)](https://github.com/sgaunet/ansible-role-forgejo-release-installer/actions?query=workflow%3ACI-${bin_name})"
  ) >> $badge_file
